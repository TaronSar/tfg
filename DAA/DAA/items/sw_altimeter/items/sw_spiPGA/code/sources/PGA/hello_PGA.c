...
/*****************************************************************************/
void pga_ldo_enable(XSpiPs *SpiInstancePtr){

	int Status;
	Status =  XSpiPs_SetSlaveSelect(SpiInstancePtr, (u8) PGA_DEVICE);
		if (Status != XST_SUCCESS) {
			xil_printf("Error PGA not Initialized\n");
			Status = XST_FAILURE;
			}
...
int pga112_init(XSpiPs *SpiInstancePtr, uint16_t gain_level){
	...
	return (pga112_set_gain(SpiInstancePtr, gain_level));
}
