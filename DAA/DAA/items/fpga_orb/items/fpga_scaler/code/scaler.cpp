...
*/
/**
 * SC: modificado para realizar downscale 1440x1080 => 640x480
 * Modificaciones:
 * 	- tamaño imagen entrada 1440x1080
 * 	- trabaja con 2pixel/clock
 * 	- versión AXILite
 * VM: modificado para generar stream de video con el formato Xilinx (UG934)
 *	Señales manejadas automáticamente por el interface HLS:
 ...
 *	Señales manejadas por el usuario:
 *	- tdata: carries the video data from the master to the slave
 *	- TODO tuser: used by AMD Video IPs to indicates the first pixel of a frame (from the master to the slave)
 *	- tlast: used by AMD Video IPs to indicates the last pixel of a line (from the master to the slave)
 *	- TODO: revisar la generación de tlast cuando rmn_num > 0; puede ser la causa de que
 *    para ciertas escalas la imagen no sea correcta
*/
...
ap_uint<WIDTH_BIT> unit_num;  //SC: número de unidades (paquetes de pixels) a procesar por línea
...
//SC
void process_cfg_axilite(ap_uint<32> p_width, ap_uint<32> p_height, ap_uint<32> p_scale, ap_uint<32> p_inv_scale);
...
#ifdef AXILITE
	#pragma HLS INTERFACE s_axilite port=p_width bundle=CONTROL_BUS
	#pragma HLS INTERFACE s_axilite port=p_height bundle=CONTROL_BUS
	#pragma HLS INTERFACE s_axilite port=p_scale bundle=CONTROL_BUS
	#pragma HLS INTERFACE s_axilite port=p_inv_scale bundle=CONTROL_BUS

	#pragma HLS INTERFACE ap_stable port=p_width
	#pragma HLS INTERFACE ap_stable port=p_height
	#pragma HLS INTERFACE ap_stable port=p_scale
	#pragma HLS INTERFACE ap_stable port=p_inv_scale
	process_cfg_axilite(p_width, p_height, p_scale, p_inv_scale);
#else
...
#pragma HLS INLINE off
    ...
    ap_ufixed<WIDTH_BIT + 8, WIDTH_BIT> unit_num_ufixed = width;  //SC: unit_num_ufixed<19,11> = width = 1440(11b).0(8b)
    unit_num_ufixed = my_ceil<ap_ufixed<WIDTH_BIT + 8, WIDTH_BIT>, WIDTH_BIT + 8, WIDTH_BIT>(unit_num_ufixed / PROCESS_NUM); 
	// SC: unit_num_ufixed= ceil(1440(11b).0(8b)/PROCESS_NUM (4)) = 360(11b).0(8b); redondea por arriba 
    unit_num = unit_num_ufixed.range(WIDTH_BIT + 7, 8); // SC: unit_num=unit_num_ufixed[18:8] parte entera de unit_num_ufixed
	// SC. 360 paquetes de 4 pixels por cada línea
...
//SC: cálculo de parámetros de configuracón
void process_cfg_axilite(ap_uint<32> p_width, ap_uint<32> p_height, ap_uint<32> p_scale, ap_uint<32> p_inv_scale)
{
#pragma HLS INLINE off
	// SC: Leemos los parámetros de control
	width = p_width;
	height = p_height;
	scale.range(15, 0) = p_scale.range(15,0);
	inv_scale.range(15, 0) = p_inv_scale.range(15,0);
	// SC: Calculamos los parámetros operativos
    new_width = width / scale;
    new_width = my_floor<ap_ufixed<WIDTH_BIT + 8, WIDTH_BIT>, WIDTH_BIT + 8, WIDTH_BIT>(new_width);
    new_height = height / scale;
    new_height = my_floor<ap_ufixed<HEIGHT_BIT + 8, HEIGHT_BIT>, HEIGHT_BIT + 8, HEIGHT_BIT>(new_height);
...
#pragma HLS UNROLL
            ...
            if ((p_cnt%width) == 0 && rmn_num == 0) // VM: to set tLAST in each row
                out.last = 1;
            else
                out.last = 0;
            outStream.write(out);
...
//#pragma HLS ARRAY_PARTITION variable = image_buf cyclic factor = 4 dim = 2  //n=4; 16 pixels
#pragma HLS ARRAY_PARTITION variable = image_buf cyclic factor = 1 dim = 2	 //n=1; 4 pixels
...
#pragma HLS UNROLL
                ...
                if (((int)p_cnt%(int)new_width) == 0 && rmn_num == 0)// VM: to set tLAST in each row
                	out.last = 1;
                else
                    out.last = 0;
                outStream.write(out);
