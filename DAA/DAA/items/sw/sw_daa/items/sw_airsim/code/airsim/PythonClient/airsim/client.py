...
#low - level control API
    ...
    def moveByMotorVectorPWMsAsync(self, motor_outputs, duration, vehicle_name = ''):
        """
        - Directly control the motors using PWM values

        Args:
            motor_outputs (List[float]): PWM values for the motors (between 0.0 to 1.0)
            duration (float): Desired amount of time (seconds), to send this command for
            vehicle_name (str, optional): Name of the multirotor to send this command to
        Returns:
            msgpackrpc.future.Future: future. call .join() to wait for method to finish. Example: client.METHOD().join()
        """
        return self.client.call_async('moveByMotorVectorPWMs', motor_outputs, duration, vehicle_name)
