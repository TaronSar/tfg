...
#ifndef msr_airsim_veronte_AdaptiveController_hpp
#define msr_airsim_veronte_AdaptiveController_hpp
...
{
public:
    explicit AdaptiveController(double mass = 0.916f)
        : m_(mass)
    {
    }
...
private:
    ...
    double m_; // mass in kg from settings (fallback to legacy value)
    ...
    void Sliding_Iconfig(double zddot, double x_desired, double x_current, double xdot, double y_desired, double y_current, double ydot, double yaw_current, double result[2][1])
    {
        ...
        result[0][0] = (xddot * sin(yaw_current) - yddot * cos(yaw_current)) * m_ / Fz;
        result[1][0] = (xddot * cos(yaw_current) + yddot * sin(yaw_current)) * m_ / Fz;
