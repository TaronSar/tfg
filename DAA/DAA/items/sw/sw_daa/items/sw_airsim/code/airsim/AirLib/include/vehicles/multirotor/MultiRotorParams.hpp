...
namespace airlib
{
    ...
    protected: //static utility functions for derived classes to use
        ...
        void setupFrameGenericN(Params& params, uint rotor_count)
        {
            if (rotor_count < 4 || rotor_count > 16)
                throw std::invalid_argument("Rotor count out of supported range for generic frame. Supported values: 4..16");

            params.rotor_count = rotor_count;
            std::vector<real_T> arm_lengths(params.rotor_count, 0.2275f);
            std::vector<real_T> arm_angles(params.rotor_count);
            std::vector<RotorTurningDirection> rotor_directions(params.rotor_count);

            const real_T angle_step_deg = 360.0f / static_cast<real_T>(params.rotor_count);
            const real_T angle_offset_deg = angle_step_deg / 2.0f;
            for (uint i = 0; i < params.rotor_count; ++i) {
                arm_angles[i] = angle_offset_deg + angle_step_deg * static_cast<real_T>(i);
                rotor_directions[i] = (i % 2 == 0)
                                          ? RotorTurningDirection::RotorTurningDirectionCCW
                                          : RotorTurningDirection::RotorTurningDirectionCW;
            }
            ...
            initializeRotors(params.rotor_poses,
                             params.rotor_count,
                             arm_lengths.data(),
                             arm_angles.data(),
                             rotor_directions.data(),
                             rotor_z);
        ...
        void setupFrameGenericPenta(Params& params)
        {
            ...
            initializeRotorQuadX(params.rotor_poses, 4, arm_lengths.data(), rotor_z);
            params.rotor_poses.emplace_back(Vector3r(-arm_lengths[4], 0, rotor_z),
                                            Vector3r(0, 0, -1),
                                            RotorTurningDirection::RotorTurningDirectionCCW);
