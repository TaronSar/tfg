...
namespace airlib
{
        ...
        : RpcLibServerBase(api_provider, server_address, port)
    {
        ...
        (static_cast<rpc::server*>(getServer()))->bind("moveByMotorVectorPWMs", [&](const std::vector<float>& motor_outputs, float duration, const std::string& vehicle_name) -> bool {
            static uint32_t dbg_rpc_count = 0;
            if ((++dbg_rpc_count % 50) == 0) {
                const float m0 = motor_outputs.size() > 0 ? motor_outputs[0] : -1.0f;
                Utils::log(Utils::stringf("[RpcServer][DBG] moveByMotorVectorPWMs vehicle='%s' n=%u dur=%f m0=%f",
                                          vehicle_name.c_str(), static_cast<unsigned int>(motor_outputs.size()), duration, m0));
            }
            return getVehicleApi(vehicle_name)->moveByMotorVectorPWMs(motor_outputs, duration);
        });
