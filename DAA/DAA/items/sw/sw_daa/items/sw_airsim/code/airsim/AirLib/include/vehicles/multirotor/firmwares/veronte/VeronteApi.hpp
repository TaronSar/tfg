...
#ifndef msr_airlib_VeronteDroneController_hpp
#define msr_airlib_VeronteDroneController_hpp
...
#include "AirSimVeronteBoard.hpp"
#include "AirSimVeronteCommLink.hpp"
#include "AirSimVeronteEstimator.hpp"
...
namespace airlib
{
    ...
    class VeronteApi : public MultirotorApiBase
    {

    public:
        VeronteApi(const MultiRotorParams* vehicle_params, const AirSimSettings::VehicleSetting* vehicle_setting)
            : vehicle_params_(vehicle_params)
        {
            ...
            board_.reset(new AirSimVeronteBoard(&params_));
            comm_link_.reset(new AirSimVeronteCommLink());
            estimator_.reset(new AirSimVeronteEstimator());

            //create firmware
            firmware_.reset(new veronte::Firmware(&params_, board_.get(), comm_link_.get(), estimator_.get()));
        }
    ...
    public: //VehicleApiBase implementation
        ...
        {
            return AirSimVeronteCommon::toGeoPoint(firmware_->offboardApi().getHomeGeoPoint());
        }
    ...
    public: //MultirotorApiBase implementation
        virtual real_T getActuation(unsigned int rotor_index) const override
        {
            ...
            static uint32_t dbg_actuation_count = 0;
            if (rotor_index == 0 && (++dbg_actuation_count % 200) == 0) {
                Utils::log(Utils::stringf("[VeronteApi][DBG] getActuation r0=%f", control_signal));
            }
    ...
    protected:
        ...
        {
            return AirSimVeronteCommon::toKinematicsState3r(firmware_->offboardApi().getStateEstimator().getKinematicsEstimated());
        }

        virtual Vector3r getPosition() const override
        {
            const auto& val = firmware_->offboardApi().getStateEstimator().getPosition();
            return AirSimVeronteCommon::toVector3r(val);
        }
        ...
        virtual Quaternionr getOrientation() const override
        {
            ...
            return AirSimVeronteCommon::toQuaternion(val);
        }
        ...
        {
            return AirSimVeronteCommon::toGeoPoint(firmware_->offboardApi().getGeoPoint());
        }
        ...
        virtual void commandMotorPWMs(float front_right_pwm, float rear_left_pwm, float front_left_pwm, float rear_right_pwm) override
        {
            ...
            typedef veronte::GoalModeType GoalModeType;
            veronte::GoalMode mode(GoalModeType::Passthrough, GoalModeType::Passthrough, GoalModeType::Passthrough, GoalModeType::Passthrough);

            veronte::Axis4r goal(front_right_pwm, rear_left_pwm, front_left_pwm, rear_right_pwm);

            std::string message;
            firmware_->offboardApi().setGoalAndMode(&goal, &mode, message);
        }
        virtual void commandMotorVectorPWMs(const std::vector<float>& motor_outputs) override
        {
            // Store the motor outputs for direct access via getActuation().
            // The use_direct_motors_ flag ensures the firmware's writeOutput() calls are ignored,
            // allowing direct motor control for any number of motors (4, 5, 6, etc.).
            board_->setDirectMotorOutputs(motor_outputs);

            static uint32_t dbg_cmd_count = 0;
            if ((++dbg_cmd_count % 50) == 0) {
                const float m0 = motor_outputs.size() > 0 ? motor_outputs[0] : -1.0f;
                const float m1 = motor_outputs.size() > 1 ? motor_outputs[1] : -1.0f;
                const float m2 = motor_outputs.size() > 2 ? motor_outputs[2] : -1.0f;
                const float m3 = motor_outputs.size() > 3 ? motor_outputs[3] : -1.0f;
                const float m4 = motor_outputs.size() > 4 ? motor_outputs[4] : -1.0f;
                Utils::log(Utils::stringf("[VeronteApi][DBG] commandMotorVectorPWMs n=%u m=[%f,%f,%f,%f,%f]",
                                          static_cast<unsigned int>(motor_outputs.size()), m0, m1, m2, m3, m4));
            }
        }

        virtual void afterTask() override
        {
            MultirotorApiBase::afterTask();
            board_->clearDirectMotorOutputs();
        }
        ...
        virtual void commandRollPitchYawZ(float roll, float pitch, float yaw, float z) override
        {
            ...
            typedef veronte::GoalModeType GoalModeType;
            veronte::GoalMode mode(GoalModeType::AngleLevel, GoalModeType::AngleLevel, GoalModeType::AngleLevel, GoalModeType::PositionWorld);

            veronte::Axis4r goal(roll, pitch, yaw, z);
        ...
        virtual void commandRollPitchYawThrottle(float roll, float pitch, float yaw, float throttle) override
        {
            ...
            typedef veronte::GoalModeType GoalModeType;
            veronte::GoalMode mode(GoalModeType::AngleLevel, GoalModeType::AngleLevel, GoalModeType::AngleLevel, GoalModeType::Passthrough);

            veronte::Axis4r goal(roll, pitch, yaw, throttle);
        ...
        virtual void commandRollPitchYawrateThrottle(float roll, float pitch, float yaw_rate, float throttle) override
        {
            ...
            typedef veronte::GoalModeType GoalModeType;
            veronte::GoalMode mode(GoalModeType::AngleLevel, GoalModeType::AngleLevel, GoalModeType::AngleRate, GoalModeType::Passthrough);

            veronte::Axis4r goal(roll, pitch, yaw_rate, throttle);
        ...
        virtual void commandRollPitchYawrateZ(float roll, float pitch, float yaw_rate, float z) override
        {
            ...
            typedef veronte::GoalModeType GoalModeType;
            veronte::GoalMode mode(GoalModeType::AngleLevel, GoalModeType::AngleLevel, GoalModeType::AngleRate, GoalModeType::PositionWorld);

            veronte::Axis4r goal(roll, pitch, yaw_rate, z);
        ...
        virtual void commandAngleRatesZ(float roll_rate, float pitch_rate, float yaw_rate, float z) override
        {
            ...
            typedef veronte::GoalModeType GoalModeType;
            veronte::GoalMode mode(GoalModeType::AngleRate, GoalModeType::AngleRate, GoalModeType::AngleRate, GoalModeType::PositionWorld);

            veronte::Axis4r goal(roll_rate, pitch_rate, yaw_rate, z);
        ...
        virtual void commandAngleRatesThrottle(float roll_rate, float pitch_rate, float yaw_rate, float throttle) override
        {
            ...
            typedef veronte::GoalModeType GoalModeType;
            veronte::GoalMode mode(GoalModeType::AngleRate, GoalModeType::AngleRate, GoalModeType::AngleRate, GoalModeType::Passthrough);

            veronte::Axis4r goal(roll_rate, pitch_rate, yaw_rate, throttle);
        ...
        virtual void commandVelocity(float vx, float vy, float vz, const YawMode& yaw_mode) override
        {
            ...
            typedef veronte::GoalModeType GoalModeType;
            veronte::GoalMode mode(GoalModeType::VelocityWorld, GoalModeType::VelocityWorld, yaw_mode.is_rate ? GoalModeType::AngleRate : GoalModeType::AngleLevel, GoalModeType::VelocityWorld);

            veronte::Axis4r goal(vy, vx, Utils::degreesToRadians(yaw_mode.yaw_or_rate), vz);
        ...
        virtual void commandVelocityZ(float vx, float vy, float z, const YawMode& yaw_mode) override
        {
            ...
            typedef veronte::GoalModeType GoalModeType;
            veronte::GoalMode mode(GoalModeType::VelocityWorld, GoalModeType::VelocityWorld, yaw_mode.is_rate ? GoalModeType::AngleRate : GoalModeType::AngleLevel, GoalModeType::PositionWorld);

            veronte::Axis4r goal(vy, vx, Utils::degreesToRadians(yaw_mode.yaw_or_rate), z);
        ...
        {
            veronte::GoalModeType controller_type_enum = static_cast<veronte::GoalModeType>(controller_type);
            ...
            case veronte::GoalModeType::AngleRate:
                ...
                break;
            case veronte::GoalModeType::AngleLevel:
                kp_axis4 = { kp[0], kp[1], kp[2], 1.0 };
                ki_axis4 = { ki[0], ki[1], ki[2], 0.0 };
                kd_axis4 = { kd[0], kd[1], kd[2], 0.0 };
                ...
                params_.gains_changed = true;
                break;
            case veronte::GoalModeType::VelocityWorld:
                ...
                params_.gains_changed = true;
                break;
            case veronte::GoalModeType::PositionWorld:
                kp_axis4 = { kp[1], kp[0], 0.0, kp[2] };
                ki_axis4 = { ki[1], ki[0], 0.0, ki[2] };
                kd_axis4 = { kd[1], kd[0], 0.0, kd[2] };
        ...
        virtual void commandPosition(float x, float y, float z, const YawMode& yaw_mode) override
        {
            ...
            typedef veronte::GoalModeType GoalModeType;
            veronte::GoalMode mode(GoalModeType::PositionWorld, GoalModeType::PositionWorld, yaw_mode.is_rate ? GoalModeType::AngleRate : GoalModeType::AngleLevel, GoalModeType::PositionWorld);

            veronte::Axis4r goal(y, x, Utils::degreesToRadians(yaw_mode.yaw_or_rate), z);
    ...
    private:
        ...
        {
            params_.default_vehicle_state = veronte::VehicleState::fromString(
                vehicle_setting.default_vehicle_state == "" ? "Armed" : vehicle_setting.default_vehicle_state);
            params_.mass = vehicle_params_->getParams().mass;
    ...
    private:
        ...
        veronte::Params params_;

        unique_ptr<AirSimVeronteBoard> board_;
        unique_ptr<AirSimVeronteCommLink> comm_link_;
        unique_ptr<AirSimVeronteEstimator> estimator_;
        unique_ptr<veronte::IFirmware> firmware_;
