    ...
    namespace msr
{
    namespace airlib
    {
        ...
        MultirotorRpcLibClient* MultirotorRpcLibClient::moveByMotorVectorPWMsAsync(const std::vector<float>& motor_outputs, float duration, const std::string& vehicle_name)
        {
            pimpl_->last_future = static_cast<rpc::client*>(getClient())->async_call("moveByMotorVectorPWMs", motor_outputs, duration, vehicle_name);
            return this;
        }
