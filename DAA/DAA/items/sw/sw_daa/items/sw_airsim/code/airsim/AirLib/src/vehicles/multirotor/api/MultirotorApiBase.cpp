...
namespace airlib
{
    ...
    bool MultirotorApiBase::moveByMotorVectorPWMs(const std::vector<float>& motor_outputs, float duration)
    {
        SingleTaskCall lock(this);
        ...
        return waitForFunction([&]() {
                   commandMotorVectorPWMs(motor_outputs);
                   return false; //keep sending until timeout
               },
                               duration)
            .isTimeout();
    }
