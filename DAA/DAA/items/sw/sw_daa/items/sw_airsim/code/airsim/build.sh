...
if [ "$(uname)" == "Darwin" ]; then
    ...
    # Ubuntu version >= 22, prefer the toolchain installed by setup.sh.
    if [ $"$VERSION" -lt "22" ]; then
        if $gcc; then
            export CC="$(pick_first_available gcc-8 gcc)"
            export CXX="$(pick_first_available g++-8 g++)"
        else
            export CC="$(pick_first_available clang-8 clang)"
            export CXX="$(pick_first_available clang++-8 clang++)"
        fi
    else
        if $gcc; then
            export CC="$(pick_first_available gcc-14 gcc-13 gcc)"
            export CXX="$(pick_first_available g++-14 g++-13 g++)"
        else
            export CC="$(pick_first_available clang-18 clang-14 clang)"
            export CXX="$(pick_first_available clang++-18 clang++-14 clang++)"
        fi
    fi
fi
