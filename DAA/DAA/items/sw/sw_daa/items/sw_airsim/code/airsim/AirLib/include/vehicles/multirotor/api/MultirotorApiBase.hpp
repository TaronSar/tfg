...
namespace airlib
{
    ...
    protected: //must be implemented
        ...
        virtual void commandMotorVectorPWMs(const std::vector<float>& motor_outputs)
        {
            unused(motor_outputs);
        }
    ...
    public: //these APIs uses above low level APIs
        ...
        virtual bool moveByMotorVectorPWMs(const std::vector<float>& motor_outputs, float duration);
