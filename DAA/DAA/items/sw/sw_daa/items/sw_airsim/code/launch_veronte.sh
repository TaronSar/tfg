...
--${BOLD}airsim_vehicle_name${END}       ${ITALICS}AirSim vehicle name for captures        ${END}[ ${ORANGE}default:${END} Veronte            ]  opt:${END} <${BLUE}str${END}>\n
...
--${BOLD}pyveronte${END}              ${ITALICS}launch AirSim-Veronte bridge process       ${END}[ ${ORANGE}default:${END} false          ]  <${BLUE}flag${END}>
...
"$SETTINGS_DIR/internal/setup.py" simpleflight
...
if [ "$SESSIONEXISTS" = "" ] ; then
	...
	if [ "$pyveronte" = "true" ] ; then
		start_process 12 'Main'.2  "airsim"           "veronte_bridge"
	fi
