...
def audit_split(split_dir: Path, show_sizes: bool):
    ...
    for ident in idents:
        ...
        sz_str = (f"{min(sizes):>7}  {max(sizes):>7}" if sizes
                  else f"{'─':>7}  {'─':>7}")
        print(f"  {ident.name:<30} {count:>5}  {sz_str}{flag}")

    print(f"  {'─'*58}")
    print(f"  {'TOTAL':<30} {sum(all_counts):>5}")
    print(f"  avg per identity: {np.mean(all_counts):.1f} | "
    ...
    if suspicious_nested:
        ...
        for n in suspicious_nested:
            print(f"    {n}")
        print("    Expected layout is split/<identity>/*.jpg, not split/operational/<identity>/*.jpg")
    ...
    if show_sizes and all_sizes:
        ...
        print(f"  Images in crop envelope: {in_crop_env}/{len(all_sizes)} ({100*in_crop_env/len(all_sizes):.0f}%)")
    ...
    if show_sizes and all_object_sizes:
        ...
        labels = ["<40px (too small)", "40-60px (far)", "60-120px (target)", "120-143px", "143-300px", ">300px (enrollment style)"]
        for (lo, hi), label in zip(buckets, labels):
        ...
        print(f"  Objects in envelope: {in_env}/{len(all_object_sizes)} ({100*in_env/len(all_object_sizes):.0f}%)")
...
def main():
    ...
    if enroll_names:
        ...
        print(f"    enrollment covers train+val identities: {'yes' if not missing_enrollment else 'no'}")
