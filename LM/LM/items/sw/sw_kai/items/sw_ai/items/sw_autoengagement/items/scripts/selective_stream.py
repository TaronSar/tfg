...
for url in urls:
    ...
    try:
        with RemoteZip(url) as rz:
            ...
            # Build a backup list with all other images if corruption appears.
            ...
            print(
                f"    [i] Found {len(target_files)} total assets. "
                f"Stride candidates: {len(strided_candidates)}"
            )
            print("    [*] Scanning for 20 clean, uncorrupted images...")
            ...
            for file_info in search_queue:
                ...
                try:
                    ...
                    uncorrupted_count += 1
                    print(
                        f"        [{uncorrupted_count}/20] Validated & Extracted: "
                        f"{os.path.basename(file_info)}"
                    )
            ...
            if uncorrupted_count < 20:
                print(
                    f"    [!] Warning: Only found {uncorrupted_count} uncorrupted files "
