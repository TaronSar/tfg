...
Usage:
    python scripts/plot_training_log.py \
        --log logs/train_color_mild_clean_15way_krobust_euclidean.log
    python scripts/plot_training_log.py \
        --log logs/train_color_mild_clean_15way_krobust_euclidean.log \
        --out graphs/results.png
"""
...
def load_dataset_info(root: Path | None) -> list[str]:
    ...
    train_pos = manifest.get(
        "train_positives", [name for name in train if not name.lower().startswith("neg_")]
    )
    train_neg = manifest.get(
        "train_negatives", [name for name in train if name.lower().startswith("neg_")]
    )
...
def save_csv(rows: list[dict[str, float]], path: Path) -> None:
    ...
    with path.open("w", newline="", encoding="utf-8") as handle:
        writer = csv.DictWriter(
            handle, fieldnames=["epoch", "loss", "train_acc", "val_acc", "seconds"]
        )
        writer.writeheader()
...
def draw_panel(
    ...
    draw.text(
        (plot_left + plot_width // 2 - 24, bottom - 24), "epoch", fill=(80, 85, 90), font=small_font
    )
    ...
    for key, color in zip(keys, colors, strict=False):
        ...
        points = map_points(
            series, plot_left, plot_top, plot_width, plot_height, value_min, value_max
        )
        if len(points) > 1:
...
def draw_summary(
    draw: ImageDraw.ImageDraw, rows: list[dict[str, float]], box: tuple[int, int, int, int]
) -> None:
    left, top, right, bottom = box
    font = nice_font(18)
    ...
    lines = [
        ...
        f"Max gap: {max_gap_row['train_acc'] - max_gap_row['val_acc']:+.3f} "
        f"at epoch {max_gap_row['epoch']}",
...
def draw_text_box(
    draw: ImageDraw.ImageDraw, title: str, lines: list[str], box: tuple[int, int, int, int]
) -> None:
    left, top, right, bottom = box
...
def main() -> None:
    ...
    parser.add_argument(
        "--log",
        default="logs/train_color_mild_clean_15way_krobust_euclidean.log",
        help="Training log produced by src.train.",
    )
    parser.add_argument(
        "--out", default=None, help="PNG path. Default: graphs/<log stem>_curves.png"
    )
    parser.add_argument(
        "--csv", default=None, help="CSV path. Default: csvs/<log stem>_metrics.csv"
    )
    parser.add_argument(
        "--data_root",
        default=None,
